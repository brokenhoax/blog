...
function PfSense() {
  const calloutContent: string[] = [
    ...
    "Thanks for sticking around and see you at the next post!",
  ];
  const images = [
    {
      id: "1",
  ...
  const toc = (
    <ol className={`orderedList ${styles.toc}`}>
      ...
      <li key="14" className="hover:text-accent">
        <Link href="/pages/pfsense#nat-nginx">NAT for Nginx Web Server</Link>
      </li>
      <li key="15" className="hover:text-accent">
        <Link href="/pages/pfsense#nat-dns-resolver">NAT for DNS Resolver</Link>
      </li>
        ...
        <section className="section motion-preset-focus">
          ...
          <div>
            ...
            <p>
              ...
              open-source project, so why not use their hardware? Sure, you
              could save some money by running pfSense on a less expensive (but
              not purpose-built) piece of hardware. This might be a good middle
              ground if your keeping a close eye on your budget, but still want
            ...
            <p>
              ...
              come in handy as your lab grows.
            </p>
          </div>
          {/* Update the Default Password */}
          <div>
            ...
            <p>
              ...
              adminstrative web interface. Once we're logged in using the
              default credentials of "admin" for the username and "pfsense" for
              the password, the first change to make to our Netgate (pfSense)
              firewall is to change the default administrator password to
              something more complex and, therefore, secure.
            </p>
            <p>
              I suggest setting a much longer password (twelve or more
              ...
              password. Once that's taken care of, we'll start configuring
              pfSense to support our lab requirements.
            </p>
          </div>
          {/* Divider */}
          <div className="divider border-b border-accent"></div>
          ...
          <div>
            ...
            <p>
              ...
              level, when configuring a physical interface, our first step will
              be to enable the interface. Next, we'll want to choose whether or
          ...
          <div>
            ...
            <p>
              Don't forget to assign your computer with a static IP address
              within the same subnet as the management interface (e.g.,
              192.168.1.2). You should know this already, though, since you've
              gone through the Netgate 4200{" "}
              <a
                href="https://docs.netgate.com/pfsense/en/latest/solutions/netgate-4200/getting-started.html"
                className="text-accent"
                target="_blank"
          ...
          <div>
            ...
            <p>
              ...
              need a new subnet. To keep things easy, we'll choose
              192.168.2.0/24 for our subnet and our IP address for this
              interface will be 192.168.2.1. Ensure the interface is enabled and
              save your changes.
            </p>
          ...
          <div>
            ...
            <p>
              We have a similar configuration for our Storage VLAN, VLAN 30.
              We'll use 192.168.30.0/24 for our IP addressing scheme for this
              VLAN and we'll assign this VLAN interface an IP address of
              192.168.30.1.
            </p>
          ...
          <div>
            ...
            <ul className="unorderedList">
              ...
              <li key="2">
                Description:{" "}
                <span className="text-accent">nginx web server</span>
              </li>
              <li key="3">
          ...
          <div>
            ...
            <p>
              Another amazing capability built in to pfSense is its ability to
            ...
            <p>
              ...
              DNS (port 53) requests from local clients destined to any external
              IP address. Those requests will be redirected to the Netgate
              appliance itself and, in the next section, we will configure
              pfSense to either resolve those requests or forward them to
          ...
          <div>
            ...
            <p>
              ...
              control over DNS such as the ability to serve resources using
              domain names instead of IP addresses. If you want to make life
              easier for yourself and your users, then having your own DNS
              Resolver is the way to go. We will configure a few “Host
              Overrides” which will effectively swap an IP address for a
              hostname. Since domain names are easier for humans to remember,
            ...
            <p>
              ...
              once. For example, if you do not plan on following the Zscaler
              Private Service Edge lab course, then there is no need to add a
              host override for it since it will not exist on your network. That
              said, let's add a host override for our Netgate appliance
              (pfSense), our Proxmox server (the MS-01), and our web server
              (Nginx).
            </p>
          ...
          <div>
            ...
            <p>
              The first thing to know is that the order of the rules matter.
              Rules are evaluated top to bottom, so be sure that you have your
              rules ordered properly or they might not work as expected.
              Typically, you have your more specific rules towards the top of
              the list and your more generalized rules towards the bottom of the
              list.
            </p>
          </div>
          ...
          <div>
            ...
            <p>
              ...
              , we are using our Netgate appliance as a DNS resolver. Since I
              only have my Lab PC hardwired to this interface, I have set the
              source to the IP address of my Lab PC. The loopback address is the
              destination, and we're only concerned with port 53 (i.e., DNS)
              traffic.
            </p>
            <p>
              The third and fourth rule are a bad idea for a production
              environment. They effectively let all IPv4 and IPv6 traffic from
              any subnets on interface 2/4 to go to any reachable destination on
              the network known to pfSense. Obviously, we would implement much
              more granular policies in our production networks; however, we
              won't have anything plugged in to this interface in our home lab,
              so, not only are the stakes low but the exposure is, too.
            </p>
          ...
          <div>
            ...
            <p>
              For now, this port is disabled, so, you shouldn't even see an
              option to configure firewall rules for this interface. In the
              future, consider enabling this interface and adding a wireless
              access point. You could then apply what you've learned in this lab
              and build out additional use cases with wireless devices (guest
              network, IoT/OT, etc.).
            </p>
          </div>
          {/* Divider */}
          ...
          <div>
            ...
            <p>
              ...
              when we set up our NAT DNS redirect policy for the Default VLAN.
              If you don't see it, be sure to revisit{" "}
              <a
          ...
          <div>
            ...
            <p>
              Just as before, we've already configured the DNS redirect rule
              when creating our NAT redirect policy; however, we need to
              configure three new firewall rules to allow specific traffic from
          ...
          <div>
            ...
            <p>
              ...
              from our lab PC to various applications and resources. We can set
              those rules up at a later date when we harden our firewall. For
              now, all you should have to do is create an "allow all" rule for
              your lab PC.
            </p>
          ...
          <div>
            ...
            <p>
              ...
              we will cover not only implementing a Terramaster NAS, but
              providing secure remote access to it using our Zscaler App
              Connector—a component of Zscaler Private Access (ZPA). Using ZPA,
              we will be able to access our lab resources and applications
